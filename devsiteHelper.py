...
def fetchGithubFile(path, revision = None):
  ...
  try:
    ...
    logging.error(f'Unable to fetch Github file: {e.code}')
    return None
  return content
...
def renderDevSiteContent(content, lang='en'):
  ...
  for video in videos:
    videoId = re.search(r'{% setvar videoId "(.*?)"', video)[1]
  ...
  for slide in slides:
    slidesId = re.search(r'{% setvar slidesId "(.*?)"', slide)[1]
  ...
  for framebox in frameboxes:
    ...
    fbOpenTag = fbContent[1]
    fbHeight = re.search(r'height="(.*?)"', fbContent[1])
    fbClass = re.search(r'class="(.*?)"', fbContent[1])
    fbContent = fbContent[2]
    fbMemcacheKey = '/framebox/' + hashlib.md5(fbContent.encode('utf-8')).hexdigest()
    replaceWith = '<iframe class="framebox inherit-locale'
    if fbClass:
      replaceWith += ' ' + fbClass[1]
    replaceWith += '" '
    replaceWith += 'style="width: 100%;'
    replaceWith += 'height:' + fbHeight[1] + ';' if fbHeight else 'height: 100px;'
    replaceWith += '" '
